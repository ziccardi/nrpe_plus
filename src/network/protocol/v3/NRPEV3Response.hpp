...
public:
    NRPEV3Response(int16_t resultCode, const std::string& message)
            : NRPEV3AbstractPacket(2, 0, resultCode, 0, std::shared_ptr<char>(messageToBuffer(message)), message.length(), std::shared_ptr<char>(new char[3]()), 3) {
        updateCRC();
    }
